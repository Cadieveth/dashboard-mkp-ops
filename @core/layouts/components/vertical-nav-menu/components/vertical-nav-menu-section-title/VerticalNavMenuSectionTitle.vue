<template>
  <v-subheader v-if="canViewVerticalNavMenuHeader(item)" v-show="showTitle">
    <span
...
export default {
  ...
  mounted() {
    if((this.isArrayIntersect(this.item.accessPage, this.$session.get('accessHumanTask'))).length == 0 && JSON.parse(this.$session.get('userData')).username != 'superadmin'){
      this.showTitle = false;
    }
  },
  methods: {
    isArrayIntersect(b,a) {
