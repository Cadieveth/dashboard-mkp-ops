  ...
  <div>
    ...
    <v-row>
      <v-col cols="12" class="mb-0">
        <v-card>
          <v-card-title>
            <span>Create Sales Order Product Service</span>
          </v-card-title>
          <v-card-text>
            <v-row>
              <v-col cols="12" class="mb-0">
                ...
                <v-tabs-items v-model="currentTab">
                  <v-tab-item class="mt-2">
                    <v-row>
                      <v-col cols="12" class="mb-0">
                        <sales-order-filter></sales-order-filter>
                      </v-col>
                      <v-col cols="12" class="mb-0">
                        <v-card outlined>
                          <v-card-title>
...
import SalesOrderFilter from "@/views/sales-order/SalesOrderFilter.vue";
...
export default {
  ...
  mounted() {
    this.$root.$on("filterSalesOrderCreate", (msg) => {
      this.form = msg;
  ...
  methods: {
    ...
    getCountData() {
      ...
      axios
        .get(
          `${themeConfig.app.sl}/sales-invoice/count-list?workflowStatus=${workflowStatus}&ouId=${ouId}&partnerId=${partnerId}&docNo=${docNo}&dateFrom=${dateFrom}&dateTo=${dateTo}`,
          config
        )
        .then((response) => {
          this.isDialogVisible = false;
    ...
    refreshDataSalesOrder() {
      ...
      axios
        .get(
          `${themeConfig.app.api_sl}/sales-order/list-released?ouId=${ouId}&partnerId=${partnerId}&docNo=${this.form.docNo}&dateFrom=${dateFrom}&dateTo=${dateTo}`,
          config
        )
        .then((response) => {
          this.isDialogVisible = false;
